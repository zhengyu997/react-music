...
import "../static/AudioPlayfont.css";
...
class AudioPlay extends Component {
  ...
  componentDidMount() {
    ...
    audio.addEventListener("timeupdate", () => {
      ...
      if (!processItemMove) {
        ...
        this.setState({
          processItemleft: `${playWidth - 4}px`,
          processPlayedwidth: `${playWidth}px`
        })
      ...
      this.setState({
        processBufferedwidth: `${bufferWidth}px`,
  ...
  setProcess = (e, key) => {
    ...
    if (key === "click" || key === "dragMove") {
      ...
      this.setState({ processItemleft: `${offsetWidth - 4}px`, processPlayedwidth: `${offsetWidth}px` });
  ...
  resetProcess = () => {
    ...
    this.setState({ processItemleft: "-4px", processPlayedwidth: "0px" });
  };
  ...
  initVolumeProcess = () => {
    ...
    this.volumeProcessCurrent.style.height = `${processLength}px`;
    // 设置进度条item
    this.volumeProcessItem.style.bottom = `${processLength - 6}px`;
    // 设置音量
    this.audio.volume = 1;
  };
  ...
  onAddFile = (e, item) => {
    e.stopPropagation();
    alert(
      `单首歌加入收藏歌单，自定义完善，歌曲id:${item.id}，歌曲名称:${item.title
      }`
    );
  };
  ...
  render() {
    // console.log("播放器",this.props.musiclists)
    const {
      currentMusic,
      isPause,
      totalTime,
    ...
    return (
      ...
      <div>
        <div
          ...
          style={{ width: this.props.WidthPlay }}
                  ...
                  >
                    <div className="process">
                      <div
                        ...
                        style={{ width: this.state.processBufferedwidth }}
                      // ref={ref => (this.processBuffered = ref)}
                      />
                      <div
                        className="progress-played"
                        style={{ width: this.state.processPlayedwidth }}
                        ref={ref => (this.processPlayed = ref)}
                      >
                        <div
                          className="process-item"
                          style={{ left: this.state.processItemleft }}
